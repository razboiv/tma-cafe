...
function getUrlState() {
  ...
  let params = null;
  if (rawParams) { try { params = JSON.parse(decodeURIComponent(rawParams)); } catch {} }
  return { dest, params };
}
...
async function getHtml(route) {
  const html = htmlCache.get(route.htmlPath) || (await route.fetchHtml());
...
export async function navigateTo(dest, params) {
  const enc = params ? encodeURIComponent(JSON.stringify(params)) : "";
  const q = `?dest=${encodeURIComponent(dest)}${enc ? `&params=${enc}` : ""}`;
  history.pushState(null, "", q + `#dest=${encodeURIComponent(dest)}${enc ? `&params=${enc}` : ""}`);
...
export function bootRouter() { getContainers(); return handleLocation(); }