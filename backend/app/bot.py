...
def handle_web_app_data(message: Message) -> None:
    ...
    invoice_link = bot.create_invoice_link(
        title="Оплата заказа",
        description="Оплата покупки в Laurel Café",
...
def handle_successful_payment(message: Message) -> None:
    """
    Срабатывает, когда Telegram подтверждает успешный платеж.
    """
    ...
    bot.send_message(
        OWNER_CHAT_ID,
        f"💰 Клиент @{message.from_user.username or 'user'} успешно оплатил заказ на {amount:.0f} ₽",
    )
...
def refresh_webhook() -> None:
    ...
    # без параметра drop_pending_updates — твоя версия библиотеки его не поддерживает
    bot.remove_webhook()
    bot.set_webhook(
